...
class PiperSec(Gtk.Application):
    ...
    def do_activate(self):
        ...
        self.win.set_default_size(760,500)
        ...
        self.action_buttons=[]
        for label,fn in [("Novo perfil",self.edit),("Editar perfil",self.edit_selected),("Conectar",self.connect),("Desconectar",self.disconnect),("Atualizar",self.refresh),("Logs",self.show_logs)]:
    ...
    def create_logs_window(self):
        ...
        self.logs_window.set_title("PiperSec — Logs de Conexão")
        self.logs_window.set_default_size(800,500)
        ...
        toolbar.pack_end(Gtk.Label(label=""),True,True,0)
    ...
    def update_status(self, connected=False, profile=None):
        if connected:
            text="Conectada: "+profile
            color="#35a854"
            icon="network-transmit-receive"
            menu="● Conectada: "+profile
        else:
            text="Desconectada"
            color="#d9534f"
            icon="network-vpn"
    ...
    def finish_vpn_action(self, command, profile, error):
        ...
        if command == "gui-connect":
            self.update_status(True,profile)
    ...
    def refresh(self):
        ...
        try:
            ...
            for p in self.profiles.values():
                self.store.append([p["name"],p["remote_address"],p["xauth_username"]])
    ...
    def refresh_logs(self):
        try:
            out=run(["status", self.selected() or ""])
            self.append_log(out)
        except Exception as e:
            self.append_log("Erro ao obter status: "+str(e))
    ...
    def append_log(self, text):
        ...
        self.logs_buffer.insert(end, text+"\n")
